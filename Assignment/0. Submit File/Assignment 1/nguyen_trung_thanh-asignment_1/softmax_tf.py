...
if __name__ == "__main__":
    ...
    # w = tf.Variable(tf.zeros([train_x.shape[1], 10]), dtype=tf.float32)
    # w = tf.Variable(tf.random_normal(shape=w_shape), dtype=tf.float32, name='W')
    w = tf.Variable(
        tf.random_normal(shape=w_shape, mean=0.0, stddev=np.sqrt(2. / np.sum(w_shape)), dtype=tf.float32, name='W'))
